    ...
    private void handleBroadcastEvent(String payload) {
        ...
        if ("minebackup save".equals(payload)) {
            serverInstance.execute(() -> {
                ...
                serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.message.remote_save.start"), null);
                boolean allLevelsSaved = serverInstance.save(true, true, true);
                if (allLevelsSaved) {
                    serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.message.remote_save.success"), null);
                } else {
                    serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.message.remote_save.fail"), null);
                }
            });
            return;
        }
        ...
        if ("pre_hot_restore".equals(eventType)) {
            ...
            serverInstance.execute(() -> {
                serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.message.restore.preparing"), null);
        ...
        if ("pre_hot_backup".equals(eventType)) {
            serverInstance.execute(() -> {
                ...
                serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.broadcast.hot_backup.request", worldName), null);
                ...
                if (!allSaved) {
                    ...
                    serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.broadcast.hot_backup.warn", worldName), null);
                }
                LOGGER.info("World saved successfully for hot backup.");
                serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.broadcast.hot_backup.complete"), null);
            });
        ...
        if (message != null) {
            serverInstance.execute(() -> serverInstance.getPlayerManager().broadcast(message, null));
        }
    }
