...
public class MineBackup implements ModInitializer {
    ...
    private void handleBroadcastEvent(String payload) {
        ...
        if ("minebackup save".equals(payload)) {
            serverInstance.execute(() -> {
                ...
                serverInstance.getPlayerManager().broadcast(
                        Text.translatable(saved ? "minebackup.message.remote_save.success" : "minebackup.message.remote_save.fail"),
                        false);
            });
            return;
        }
        ...
        if (("backup_success".equals(eventType) || "backup_failed".equals(eventType)) && saveFrozen) {
            serverInstance.execute(() -> {
                ...
                serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.broadcast.autosave.resumed"), false);
            });
        }

        Text message = switch (eventType) {
    ...
    private void broadcastDedicatedRestoreUnsupported() {
        ...
        serverInstance.execute(() -> {
            serverInstance.getPlayerManager().broadcast(
                    Text.translatable("minebackup.message.restore.unsupported_dedicated"), false);
            serverInstance.getPlayerManager().broadcast(buildPluginLinkMessage(), false);
        });
    }

    private Text buildPluginLinkMessage() {
        return Text.translatable("minebackup.message.plugin_link_prefix")
                .append(Text.literal(PLUGIN_GUIDE_URL).styled(style -> style
                        .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, PLUGIN_GUIDE_URL))
    ...
    private void handlePreHotRestore(Map<String, String> eventData) {
        serverInstance.execute(() -> {
            ...
            if (!saved) {
                ...
                serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.message.restore.failed"), false);
    ...
    private void handleRestoreFinished(Map<String, String> eventData, String eventType) {
        ...
        if (!"success".equals(status)) {
            if (serverInstance != null) {
                serverInstance.execute(() -> serverInstance.getPlayerManager().broadcast(
                        Text.translatable("minebackup.message.restore.failed_status"), false));
            }
    ...
    private void handleRestoreCancelled(Map<String, String> eventData) {
        ...
        if (serverInstance != null) {
            serverInstance.execute(() -> serverInstance.getPlayerManager().broadcast(
                    Text.translatable("minebackup.message.restore.failed"), false));
        }

        if (serverInstance != null && !serverInstance.isDedicated()) {
            MineBackupClient.resetRestoreState();
        } else {
            HotRestoreState.reset();
        }
    ...
    private void checkFreezeTimeout() {
        ...
        unfreezeAutoSave(serverInstance);
        serverInstance.getPlayerManager().broadcast(Text.translatable("minebackup.broadcast.autosave.timeout"), false);
    }
