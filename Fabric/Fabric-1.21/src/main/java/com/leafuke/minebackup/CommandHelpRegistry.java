...
public final class CommandHelpRegistry {
    ...
    public static CompletableFuture<Suggestions> suggestCommands(SuggestionsBuilder builder) {
        ...
        for (HelpEntry entry : ENTRIES) {
            if (matches(entry.name(), remaining)) {
                builder.suggest(entry.name(), new LiteralMessage(Text.translatable(entry.summaryKey()).getString()));
            }
            for (String alias : entry.aliases()) {
                if (matches(alias, remaining)) {
                    builder.suggest(alias, new LiteralMessage(Text.translatable("minebackup.help.suggest.alias", entry.name()).getString()));
                }
            }
        }
    ...
    public static Text buildRootHelp() {
        ...
        for (HelpEntry entry : ENTRIES) {
            text.append(Text.translatable("minebackup.help.root.entry",
    ...
    public static Text buildCommandHelp(String requestedName) {
        HelpEntry entry = find(requestedName);
        if (entry == null) {
            return Text.translatable("minebackup.help.command.unknown", requestedName);
        }

        MutableText text = Text.translatable("minebackup.help.command.title", entry.name());
        text.append(Text.translatable("minebackup.help.command.summary", Text.translatable(entry.summaryKey())));
        text.append(Text.translatable("minebackup.help.command.usage", "/mb " + entry.name() + formatUsage(entry.usage())));
        if (!entry.aliases().isEmpty()) {
            text.append(Text.translatable("minebackup.help.command.aliases", String.join(", ", entry.aliases())));
        }
